...
def bbox_iou(
    ...
    if xywh:
        ...
        # widths/heights for union
        w1_u = (b1_x2 - b1_x1).clamp(min=0) + eps
        h1_u = (b1_y2 - b1_y1).clamp(min=0) + eps
        w2_u = (b2_x2 - b2_x1).clamp(min=0) + eps
        h2_u = (b2_y2 - b2_y1).clamp(min=0) + eps
    else:
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
        # widths/heights for union (robust)
        w1_u = (b1_x2 - b1_x1).clamp(min=0) + eps
        h1_u = (b1_y2 - b1_y1).clamp(min=0) + eps
        w2_u = (b2_x2 - b2_x1).clamp(min=0) + eps
        h2_u = (b2_y2 - b2_y1).clamp(min=0) + eps
    ...
    # Union Area
    union = w1_u * h1_u + w2_u * h2_u - inter + eps
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:
            ...
            if CIoU:
                # reuse robust widths/heights
                v = (4 / math.pi**2) * ((w2_u / h2_u).atan() - (w1_u / h1_u).atan()).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + v * alpha)
            return iou - rho2 / c2
        c_area = (cw * ch).clamp(min=eps)
